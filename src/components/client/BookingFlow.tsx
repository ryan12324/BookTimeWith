"use client";

import { useState } from "react";
import { usePublishedConfig } from "@/lib/store";
import { fmtDuration } from "@/lib/format";
import { BOOKING_DAYS, OWNER_NAME, SLOT_SETS } from "@/lib/mock";
import { StatusBadge } from "@/components/ui";
import { CardShell } from "./CardShell";
import { DayTabs, SlotGrid } from "./Picker";
import { T } from "@/lib/tokens";

type Step = "pick" | "details" | "done";

/**
 * Public booking flow (booktimewith.link/handle). Pick a day + slot → two-field
 * form → confirmed. Reads the owner's published service/length/location so the
 * page reflects whatever the owner set.
 */
export function BookingFlow() {
  const cfg = usePublishedConfig();
  const service = cfg.service.trim() || "Session";
  const serviceLine = `${service} · ${fmtDuration(cfg.duration)}`;
  const needsAddress = cfg.location === "theirs";

  const [step, setStep] = useState<Step>("pick");
  const [day, setDay] = useState(0);
  const [slot, setSlot] = useState(-1);
  const [name, setName] = useState("");
  const [email, setEmail] = useState("");
  const [address, setAddress] = useState("");

  const slots = SLOT_SETS[day];
  const hasSlot = slot >= 0;
  const pickedTime = hasSlot ? slots[slot] : "";
  const dayFull = BOOKING_DAYS[day].full;
  const dayShort = dayFull.split(" ")[0];
  const canConfirm = name.trim() && email.trim() && (!needsAddress || address.trim());

  return (
    <div className="mx-auto flex justify-center px-0 pt-0 sm:px-6 sm:pt-9">
      <CardShell ownerName={OWNER_NAME} serviceLine={serviceLine}>
        {step === "pick" && (
          <div className="px-[26px] pb-[26px] pt-5">
            <DayTabs days={BOOKING_DAYS} selected={day} onPick={(i) => { setDay(i); setSlot(-1); }} />
            <div className="mt-[18px]">
              <SlotGrid slots={slots} selected={slot} onPick={setSlot} />
            </div>
            <button
              type="button"
              disabled={!hasSlot}
              onClick={() => hasSlot && setStep("details")}
              className="sticky bottom-3 z-10 mt-[18px] w-full rounded-chip py-[13px] text-center font-sans text-[14px] font-semibold text-paper sm:static"
              style={{ background: hasSlot ? T.bronze : T.disabled }}
            >
              {hasSlot ? `Book ${dayShort} at ${pickedTime} →` : "Pick a time"}
            </button>
            <div className="mt-[10px] text-center font-sans text-[11.5px] text-faint">
              No account needed. Ever.
            </div>
          </div>
        )}

        {step === "details" && (
          <div className="px-[26px] pb-[26px] pt-5">
            <div className="rounded-chip border border-hairline bg-paper px-4 py-3 font-sans text-[13px] text-body">
              {service} · {dayFull} at {pickedTime}
              <button
                type="button"
                onClick={() => setStep("pick")}
                className="ml-[6px] font-semibold text-bronze"
              >
                change
              </button>
            </div>

            <Field label="Your name" value={name} onChange={setName} placeholder="Alex Martin" />
            <Field label="Email" value={email} onChange={setEmail} placeholder="alex@example.com" type="email" />
            {needsAddress && (
              <Field
                label="Your address"
                value={address}
                onChange={setAddress}
                placeholder="Where should Dana come?"
              />
            )}

            <button
              type="button"
              disabled={!canConfirm}
              onClick={() => canConfirm && setStep("done")}
              className="sticky bottom-3 z-10 mt-5 w-full rounded-chip py-[13px] text-center font-sans text-[14px] font-semibold text-paper sm:static"
              style={{ background: canConfirm ? T.ink : T.disabled }}
            >
              Confirm booking
            </button>
            <div className="mt-[10px] text-center font-sans text-[11.5px] text-faint">
              Two fields. That&apos;s the whole form.
            </div>
          </div>
        )}

        {step === "done" && (
          <div className="px-[26px] py-[34px] text-center">
            <StatusBadge variant="done" />
            <div className="mt-[18px] font-serif text-[24px] tracking-[-.01em]">Booked.</div>
            <p className="mt-2 font-sans text-[13.5px] leading-[1.6] text-body">
              {service} with Dana · {dayFull} at {pickedTime}.
            </p>
            <p className="mt-[6px] font-sans text-[13px] text-body">
              {needsAddress
                ? `Dana comes to you: ${address.trim() || "your address"}`
                : `At ${cfg.ownerAddress.trim() || "Dana's practice"} — address is in your confirmation.`}
            </p>
            <p className="mt-[6px] font-sans text-[12px] text-faint">
              Confirmation and a reminder are on their way to {email.trim() || "your inbox"}.
            </p>
            <button
              type="button"
              onClick={() => { setStep("pick"); setSlot(-1); }}
              className="mt-[22px] font-sans text-[12.5px] font-semibold text-bronze"
            >
              Book another time
            </button>
          </div>
        )}
      </CardShell>
    </div>
  );
}

function Field({
  label,
  value,
  onChange,
  placeholder,
  type = "text",
}: {
  label: string;
  value: string;
  onChange: (v: string) => void;
  placeholder: string;
  type?: string;
}) {
  return (
    <>
      <div className="mb-2 mt-[14px] font-sans text-[11.5px] font-semibold uppercase tracking-label text-faint">
        {label}
      </div>
      <input
        type={type}
        value={value}
        onChange={(e) => onChange(e.target.value)}
        placeholder={placeholder}
        className="w-full rounded-chip border border-line px-[15px] py-[13px] font-medium text-ink outline-none"
        style={{ fontSize: 14.5 }}
      />
    </>
  );
}
