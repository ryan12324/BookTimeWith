  ...
  const [services, connections, bookingCounts] = await Promise.all([
    ownerIds.length
      ? db
          ...
          .where(sql`${schema.services.ownerId} = ANY(${ownerIds})`)
      : [],
    ownerIds.length
      ? db
          .select({
          ...
          .where(sql`${schema.calendarConnections.ownerId} = ANY(${ownerIds})`)
      : [],
    ownerIds.length
      ? db
          .select({
          ...
          .where(
            sql`${schema.bookings.ownerId} = ANY(${ownerIds}) AND ${schema.bookings.status} = 'confirmed'`,
          )
